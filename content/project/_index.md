---
title: "2411 N. Glassell Warehouse: Project Specs & Timeline"
description: "2411 Glassell project specs and timeline: full building specifications, entitlements, and developer information for the proposed 298,988 SF warehouse at 2411 North Glassell Street, Orange, CA."
ShowToc: true
TocOpen: false
---

Rexford Industrial Realty, Inc. is proposing to demolish three existing office buildings (191,127 SF total) at 2411 North Glassell Street and redevelop the site with a single industrial warehouse building.

## Project Specifications

### Location & Site

| Property | Detail |
|----------|--------|
| **Address** | 2411 North Glassell Street, Orange, CA 92866 |
| **Corner Location** | Southeast corner of Glassell Street and Fletcher Avenue |
| **Site Size** | 12.1 net acres |
| **Assessor's Parcel Numbers** | 374-381-01 & 374-381-02 |
| **Zoning** | M-2 (Industrial Manufacturing) |
| **General Plan Designation** | Light Industrial (LI) |
| **Max Height Allowed** | 45 feet |

### Building & Operations

| Specification | Detail |
|---------------|--------|
| **Total Building Size** | 298,988 square feet |
| **Warehouse Space** | 260,118 SF (latest); 189,988 SF (analysis assumed) |
| **Manufacturing Space** | 20,000 SF (latest); 109,000 SF (analysis assumed) |
| **Office Space** | 18,800 SF (9,400 SF ground floor + 9,400 SF mezzanine) |
| **Building Height** | One story + mezzanine (within 45-foot limit) |
| **Loading Dock Doors** | 36 (Option 2 configuration) |
| **Parking Spaces** | 380 total (297 standard, 5 accessible, 2 van, 76 EV) |
| **Bicycle Parking** | 20 spaces (14 short-term, 6 long-term) |

### Traffic & Operations

| Specification | Detail |
|---------------|--------|
| **Operating Hours** | 24 hours per day, 7 days per week (24/7) |
| **Daily Vehicle Trips** | 843 total; 556 net new (≈163 trucks, 810 PCE-weighted) |
| **AM Peak Hour Trips** | 106 vehicle trips (139 PCE) |
| **PM Peak Hour Trips** | 115 vehicle trips (147 PCE) |
| **Truck Driveway Access** | Southern driveway on Glassell Street (40 ft wide) |
| **Passenger Vehicle Access** | Northern driveway on Glassell Street (30 ft, right-in/right-out only) |
| **Fire Access** | Separate 28-foot driveway via Fletcher Avenue (northeast) |
| **Expected Opening Year** | 2027 |

## Current Project Development

### What's Being Demolished

The site currently contains three 2-story concrete tilt-up office buildings totaling 191,127 SF in a circular campus layout — occupied office buildings with existing tenants.

### What's Being Built

A single-story industrial warehouse (plus mezzanine) designed for mixed warehouse and light manufacturing uses, with extensive loading dock operations, 24-hour truck traffic with heavy diesel vehicles, round-the-clock employee and operational activity, and significant outdoor lighting and security systems.

## Entitlements Required

| Entitlement | Case Number | Type |
|-------------|-------------|------|
| **Conditional Use Permit** | CUP 25-0013 | Required for warehouse use in M-2 zone |
| **Major Site Plan Review** | SP-1180 | Required for projects over 100,000 SF |
| **Design Review** | DR-5154 | Aesthetic review of building design |
| **Lot Line Adjustment** | LLA 2025-03 | Reconfiguring parcel boundaries |
| **Environmental Review** | ENV25-0002 | CEQA analysis |

### Lead Agency

**City of Orange** — Community Development Department, Planning Division
300 East Chapman Avenue, Orange, CA 92866 · (714) 744-7220

## Developer & Applicant

### Rexford Industrial Realty, Inc.

| Contact | Detail |
|---------|--------|
| **Applicant** | Rexford Industrial Realty, Inc. |
| **Address** | 555 Anton Blvd, Suite 910, Costa Mesa, CA 92626 |
| **Contact Person** | Luis Gomez, Vice President, Development |
| **Phone** | (424) 465-2061 |
| **Email** | LGomez@rexfordindustrial.com |
| **Outreach Email** | 2411NGlassell@gmail.com *(Rexford's outreach email — not the neighborhood group's)* |

Rexford Industrial Realty is a publicly traded real estate investment trust (REIT) specializing in industrial warehouse properties throughout Southern California. This project is consistent with their business model of acquiring land in industrial zones and maximizing density through warehousing operations.

## Environmental Review Process & Status

On **April 16, 2026**, the City of Orange released a **Draft Initial Study/Mitigated Negative Declaration (IS/MND)** for public review, with a 20-day public comment period (April 16 – May 6, 2026).

The IS/MND is available on the [City of Orange Planning Division website](https://www.cityoforange.org/our-city/departments/community-development/planning-division/current-projects) and in our [Evidence & Documents library](/evidence/).

An IS/MND is a streamlined environmental review (not a full Environmental Impact Report). The City's consultants concluded that the project would have **no significant environmental impacts** if certain mitigation measures are implemented.

**Contested impacts** (impacts the City claims can be "mitigated to less than significant"):

- Air quality and diesel particulate matter emissions
- Noise from 24/7 operations and dock activities
- Traffic and Level of Service impacts
- Greenhouse gas emissions

We have identified substantial evidence in the City's own documents that contradicts the "no significant impacts" conclusion. [See 2411 Glassell's key environmental issues →](/issues/)

## Surrounding Land Uses & Sensitive Receptors

### What's Around the Site

| Direction | Existing Use | Distance |
|-----------|--------------|----------|
| **North** | Fletcher Avenue, then single-family residences | 54 feet |
| **South** | Former railroad spur, then warehouses/industrial | Adjacent |
| **East** | BNSF railroad (active freight line) | Adjacent |
| **West** | Glassell Street, then commercial/industrial | 0 feet (frontage) |

### Sensitive Land Uses

{{< callout type="alert" title="Fletcher Elementary School" icon="📍" >}}
- **Distance**: 0.25 miles (quarter-mile) north of the site
- **Students**: ~497 (TK–6)
- **Programs**: Mandarin Language Academy & GATE Academy
- **Address**: 515 W. Fletcher Avenue
- **Significance**: Closest sensitive receptor for air quality and noise impacts; most vulnerable population (children)
{{< /callout >}}

### Residential Homes and Sensitive Receptors — 54 Feet (Immediate North Boundary)

Single-family residences along Fletcher Avenue (zoned R-1-7 and R-1-8, Low Density Residential) are home to families with young children and infants. These residences sit 54 feet from the northern project boundary and are occupied by sensitive receptors as defined by CARB and OEHHA guidance — particularly children during critical developmental windows (in utero through age 18). The 24/7 warehouse operations (truck traffic, diesel particulate matter, nighttime noise, and light pollution) will expose sleeping children to:

- Nighttime noise (dock operations, backup alarms, refrigeration units) during hours when children's sleep is essential for neurodevelopment and immune function (10 PM–7 AM)
- Cumulative diesel exhaust and fine particulate exposure at a residence only 54 feet away, compounding exposure from the nearby school and existing freeway traffic
- Sleep disruption and circadian rhythm disturbance in a developmental cohort most sensitive to chronic noise stress and air quality impacts

Families in these homes are not transient tenants; they are choosing to raise children in Orange's residential neighborhoods based on existing land use patterns. The project converts a light-industrial site adjacent to single-family residential into 24/7 heavy-industrial operations — a fundamental incompatibility with the sensitive-receptor population now at risk.

## Timeline: Project Review Process

| Date | Event |
|------|-------|
| **April 16, 2026** | City releases Draft IS/MND for public review |
| **April 16 – May 6, 2026** | Public comment period (20 days) |
| **May 6, 2026** | Comment period closes |
| *Late May / early June 2026* | City prepares responses to comments |
| **September 8, 2026** | City Council meeting, 6:00 PM (public comment open; project not on agenda) |
| **September 10, 2026** | Planning Commission meeting, 5:30 PM (public comment open; project not on agenda) |
| **September 21, 2026** | **Planning Commission hearing on the project**, 5:30 PM (tentative; 10-day notice) |
| **10–15 days after PC approval** | Appeal window to City Council (if decision is appealed) |

### How the Process Works

**Step 1: Initial Study / Mitigated Negative Declaration (current)** — City determines if the project has "significant environmental effects." If IS/MND is approved, project moves forward under streamlined review. If a "fair argument" of significant effects is shown, an EIR must be prepared.

**Step 2: Planning Commission hearing (next)** — Commission reviews staff report, IS/MND, and public comments. Public comment is taken. Commission votes to approve, deny, or continue the project.

**Step 3: City Council appeal (if applicable)** — If denied by PC, applicant can appeal to Council. If approved by PC, community can appeal to Council. Council makes the final decision.

**Step 4: Final environmental decision** — If PC approves under IS/MND, the project can move forward with streamlined review. If IS/MND is overturned or an EIR is demanded, a full Environmental Impact Report is required.

## Key Questions About This Project

### Why should this go through an EIR, not just an IS/MND?

An EIR is required if there is **substantial evidence** that the project may have a **significant environmental effect**. We believe the City's own technical studies contain evidence of significant impacts that have not been adequately addressed. [See the Evidence →](/evidence/)

### What does "mitigated" mean?

"Mitigated" means the City claims the project's impacts can be reduced to "less than significant" by implementing mitigation measures (e.g., sound walls, truck routing restrictions, air filtration). However, many of these mitigation measures are vague, unenforceable, or rely on future regulatory compliance rather than concrete project controls.

### Can conditions be changed?

Yes. Even if the project is approved, the Planning Commission can impose **Conditions of Approval** that restrict operating hours, limit truck traffic and routing, require sound walls or other noise controls, mandate air quality monitoring, establish community benefit agreements, and require performance bonds to ensure compliance. The stronger the conditions, the less the impact on the neighborhood.

### What about the 36 loading docks?

The health risk assessment (HRA) modeled only 5 loading dock doors operating simultaneously. However, **Footnote 1 of the Traffic Impact Analysis confirms the current site plan shows 36 loading dock doors** (Option 2 configuration) — a major discrepancy that may mean the air quality analysis understates actual emissions. [More on air quality →](/issues/#issue-1-air-quality-and-health-risk-assessment)

### Is the project legal?

Yes — the project is legally consistent with the M-2 (Industrial Manufacturing) zoning and Light Industrial General Plan designation. The question is not whether it's "legal," but whether its **environmental impacts have been adequately studied and disclosed**.

## Developer Strategy & Timeline

Rexford Industrial has been acquiring industrial sites throughout Orange County. This site is zoned for industrial use (M-2), large enough for a 298,988 SF building, well-located for truck traffic near freeways and major arterials, and primed for demolition (existing office buildings are older).

Rexford makes money by leasing warehouse space to fulfillment centers, logistics companies, and cold storage operators, operating as a REIT that requires high occupancy and rental income, and maximizing truck traffic and operational intensity to command higher rents.

## What's at Stake?

If this project is approved without proper environmental review and strict conditions:

- **Children at Fletcher Elementary** are exposed to increased diesel emissions and health risks during their school day
- **Residents along Fletcher Avenue** lose sleep from 24/7 truck traffic and dock operations
- **Traffic safety** is compromised on already-congested local streets
- **Neighborhood character** shifts permanently from office campus to industrial warehouse

**This is a precedent-setting decision.**

## Next Steps

- **Review the technical evidence**: [See Issues & Evidence](/issues/)
- **Prepare your public comment**: [How to submit public comment at the Planning Commission](/testify/#how-to-prepare-your-public-comment)
- **Join the group**: [Get Involved](/testify/#how-to-join-the-group)
- **Stay informed**: [Subscribe to email updates](/#stay-informed)

{{< share text="The Project: 298,988 SF warehouse proposed at 2411 N. Glassell St, Orange CA — 24/7 operations, 54 feet from homes, quarter-mile from Fletcher Elementary." >}}
